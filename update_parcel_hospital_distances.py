...
try:
    conn = psycopg2.connect("dbname='spatial' user='postgres' host='localhost' password='ar123'")
except:
...
while row is not None:
    ...
    parid2 = str(row2[0])
    distance = row2[2]
    sql3 = "update volusia.parcel p1 set hdistance = " + str(distance) + " where p1.parid=" + parid + ";"
    cur3.execute(sql3)
    # print(sql3)
    if i%10000 == 0:
        print(i)
